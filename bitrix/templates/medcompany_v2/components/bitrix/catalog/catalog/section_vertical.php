                ...
                if ($isFilter && !isset($isHaveSections)): ?>
                    <div class="bx-sidebar-block">
                        ...
                        if (!isset($isHaveSections)) {
                            $GLOBALS['mkPreFilter'] = array(">CATALOG_PRICE_3" => 0);
...
$GLOBALS['arrFilter'] = array_merge(
    isset($GLOBALS['arrFilter']) && is_array($GLOBALS['arrFilter']) ? $GLOBALS['arrFilter'] : array(),
    array(
        "IBLOCK_ID" => $arParams["IBLOCK_ID"],
        "ACTIVE" => "Y",
        ">CATALOG_PRICE_3" => 0,
...
// Sorting chosen by the visitor (whitelisted)
$mkSorts = array(
    'popular' => array('Популярные', 'PREVIEW_PICTURE', 'DESC', 'SORT', 'ASC'),
    'price_asc' => array('Сначала дешёвые', 'CATALOG_PRICE_3', 'ASC', 'PREVIEW_PICTURE', 'DESC'),
    'price_desc' => array('Сначала дорогие', 'CATALOG_PRICE_3', 'DESC', 'PREVIEW_PICTURE', 'DESC'),
    'name' => array('По названию', 'NAME', 'ASC', 'PREVIEW_PICTURE', 'DESC'),
    'new' => array('Новинки', 'ID', 'DESC', 'SORT', 'ASC'),
);
$mkSort = isset($_GET['sort'], $mkSorts[$_GET['sort']]) ? $_GET['sort'] : 'popular';
$mkLimits = array(24, 48, 96);
$mkLimit = isset($_GET['limit']) && in_array((int)$_GET['limit'], $mkLimits, true) ? (int)$_GET['limit'] : 24;
if (!isset($isHaveSections)): ?>
    <div class="mk-sortbar" role="navigation" aria-label="Сортировка товаров">
        <span class="mk-sortbar__label">Сортировка:</span>
        <div class="mk-sortbar__options">
            <? foreach ($mkSorts as $mkKey => $mkItem): ?>
                <a class="mk-sortbar__opt<?= $mkKey === $mkSort ? ' is-active' : '' ?>"
                   <?= $mkKey === $mkSort ? 'aria-current="true"' : '' ?>
...
<? endif;
            ...
            if (!isset($isHaveSections)) {
                $intSectionID = $APPLICATION->IncludeComponent(
                    ...
                    "IBLOCK_TYPE" => $arParams["IBLOCK_TYPE"],
                    "IBLOCK_ID" => $arParams["IBLOCK_ID"],
                    "ELEMENT_SORT_FIELD" => $mkSorts[$mkSort][1],
                    "ELEMENT_SORT_ORDER" => $mkSorts[$mkSort][2],
                    "ELEMENT_SORT_FIELD2" => $mkSorts[$mkSort][3],
                    "ELEMENT_SORT_ORDER2" => $mkSorts[$mkSort][4],
                    "PROPERTY_CODE" => (isset($arParams["LIST_PROPERTY_CODE"]) ? $arParams["LIST_PROPERTY_CODE"] : []),
                    "PROPERTY_CODE_MOBILE" => $arParams["LIST_PROPERTY_CODE_MOBILE"],
                    ...
                    "CACHE_GROUPS" => $arParams["CACHE_GROUPS"],
                    "SET_TITLE" => $arParams["Y"],
                    "MESSAGE_404" => $arParams["~MESSAGE_404"],
                    "SET_STATUS_404" => $arParams["SET_STATUS_404"],
                    "SHOW_404" => $arParams["SHOW_404"],
                    "FILE_404" => $arParams["FILE_404"],
                    "DISPLAY_COMPARE" => $arParams["USE_COMPARE"],
                    "PAGE_ELEMENT_COUNT" => $mkLimit,
