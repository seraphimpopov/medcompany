...
$showPriceOnRequest = MedcompanyPriceRequest::hidePriceItem($item);
$noCart = MedcompanyPriceRequest::noCartItem($item);
?>
...
<div class="product-item">
    ...
    justify-content: space-between;">
        <? /* if ($item['DISPLAY_PROPERTIES']['CML2_ARTICLE']) { */ ?><!--
            <div class="product-item-info-container product-item-hidden">
                <dl class="product-item-properties">
                    <dt class="hidden-xs">
                        <?php /*= $item['DISPLAY_PROPERTIES']['CML2_ARTICLE']['NAME'] */ ?>
                    </dt>
                    <dt class="hidden-xs">
                        <?php /*= $item['DISPLAY_PROPERTIES']['CML2_ARTICLE']['DISPLAY_VALUE'] */ ?>
                    </dt>
                </dl>
            </div>
        --><? /* } */ ?>
        <button data-id="<?= $item['ID'] ?>" type="button" class="bazarow_add_favor">
            <svg class="icon">
...
<?php } else { ?>
                ...
                case 'quantity':
                    if (!$haveOffers) {
                        if ($actualItem['CAN_BUY'] && $arParams['USE_PRODUCT_QUANTITY'] && !$noCart) {
                            ?>
                            <div class="product-item-info-container product-item-hidden" data-entity="quantity-block">
                                <div class="product-item-amount">
                                    <div>
                                        <div class="product-item-amount-field-container" style="border: 1px solid #e5e5ed">
                        ...
                        if ($arParams['USE_PRODUCT_QUANTITY']) {
                            ...
                            <div class="product-item-info-container product-item-hidden" data-entity="quantity-block">
                                <div class="product-item-amount">
                                    <div>
                                        <div class="product-item-amount-field-container" style="border: 1px solid">
                                        <span class="product-item-amount-field-btn-minus no-select"
                                              id="<?= $itemIds['QUANTITY_DOWN'] ?>"></span>
                                            <input class="product-item-amount-field" id="<?= $itemIds['QUANTITY'] ?>"
                                                   type="number"
...
<?php if ($noCart) { ?>
    <div class="product-item-button-container" id="<?= $itemIds['BASKET_ACTIONS'] ?>">
        <button class="btn btn-default <?= $buttonSizeClass ?>"
                onclick="window.location.href='tel:+74852429560';"
                rel="nofollow">
            Сделать звонок
        </button>
    </div>
<?php } else { ?>
                                <div class="product-item-button-container" id="<?= $itemIds['BASKET_ACTIONS'] ?>">
                                    <button class="btn btn-default <?= $buttonSizeClass ?>" id="<?= $itemIds['BUY_LINK'] ?>"
						...
						<? }?>
                                ...
                                <div class="product-item-button-container">
                                    ...
                                    ?>
                                    <button style="" class="notavailable btn btn-link <?= $buttonSizeClass ?>"
                                       id="<?= $itemIds['NOT_AVAILABLE_MESS'] ?>" href="tel: +74852429560"
                                       rel="nofollow">
                            ...
                            <div class="product-item-info-container product-item-hidden" data-entity="props-block">
                                <dl class="product-item-properties">
                                    ...
                                    foreach ($item['DISPLAY_PROPERTIES'] as $code => $displayProperty) {
                                        */
                    ?>
                                        <dt<?php /*= (!isset($item['PROPERTY_CODE_MOBILE'][$code]) ? ' class="hidden-xs"' : '') */
                    ?>>
                                            <?php /*= $displayProperty['NAME'] */
                    ?>
                                        </dt>
                                        <dd<?php /*= (!isset($item['PROPERTY_CODE_MOBILE'][$code]) ? ' class="hidden-xs"' : '') */
                    ?>>
                                            <?php /*= (is_array($displayProperty['DISPLAY_VALUE'])
                    ...
                    ?>
                        ...
                        if ($arParams['ADD_PROPERTIES_TO_BASKET'] === 'Y' && !empty($item['PRODUCT_PROPERTIES'])) {
                            */
                    ?>
                            <div id="<?php /*= $itemIds['BASKET_PROP_DIV'] */
                    ?>" style="display: none;">
                                <?/*
                    ...
                    ?>
                                        <input type="hidden"
                                               name="<?php /*= $arParams['PRODUCT_PROPS_VARIABLE'] */
                    ?>[<?php /*= $propID */
                    ?>]"
                                               value="<?php /*= htmlspecialcharsbx($propInfo['ID']) */
                    ?>">
                                        <?/*
                    ...
                    ?>
                                            <tr>
                                                <td><?php /*= $item['PROPERTIES'][$propID]['NAME'] */
                    ?></td>
                                                <td>
                                                    <?/*
                                                    if (
                    ...
                    ?>
                                                        ...
                                                        */
                    ?>
                                                        <select name="<?php /*= $arParams['PRODUCT_PROPS_VARIABLE'] */
                    ?>[<?php /*= $propID */
                    ?>]">
                                                            <?/*
                                                            foreach ($propInfo['VALUES'] as $valueID => $value) {
