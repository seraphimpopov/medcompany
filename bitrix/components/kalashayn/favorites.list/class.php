...
class bazarowFavoritesListComponent extends CBitrixComponent
{
	/**
	...
	protected function setTitle()
	{
		...
		if ($this->arParams["SET_TITLE"])
			$APPLICATION->SetTitle(Loc::getMessage("bazarow_FAVORITES_DEFAULT_TITLE"));
	}
	...
	protected function performActionDeleteFavor()
	{
		if (strlen($this->requestData["ID"]) && $this->requestData["DELETE_FAVOR"] == "Y")
		{
			if($id = $this->getRealId($this->requestData["ID"]))
				...
				throw new Main\SystemException(Loc::getMessage('bazarow_FAVORITES_CANNOT_DELETE_FAVORITES'));
		}
	}
	...
	{
		$res = \bazarow\Favorites\FavoritesTable::getById($id);
  		$return = 0;
	...
	protected function deleteFavor($id)
	{
		...
		{
			$DB->StartTransaction();
			$result = \bazarow\Favorites\FavoritesTable::delete($id);
			if(!$result->isSuccess())
			{
			  $DB->Rollback();
			  throw new Main\SystemException(Loc::getMessage('bazarow_FAVORITES_CANNOT_DELETE_FAVORITES'));
	...
	protected function obtainDataFavoritess()
	{
		...
		$this->dbQueryResult['FAVORITES'] = \bazarow\Favorites\FavoritesTable::getList(
			array(
