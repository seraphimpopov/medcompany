...
class bazarowFavoritesAddComponent extends CBitrixComponent
{
	...
	{
		if (!Loader::includeModule('bazarow.favorites'))
			throw new Main\SystemException(Loc::getMessage("bazarow_FAVORITES_MODULE_NOT_INSTALL"));
	...
	protected function formatResult()
	{
		...
		/** find favor */
		$favorDb = \bazarow\Favorites\FavoritesTable::getList(array(
	...
	public static function getCookieUserID(){
		...
		if($cookie_user_id = $APPLICATION->get_cookie("bazarow_COOKIE_USER_ID")){
			return $cookie_user_id;
		}else{
			$cookie_user_id = md5(time().randString(10));
			$APPLICATION->set_cookie("bazarow_COOKIE_USER_ID", $cookie_user_id);
	...
	{
		if($this->requestData['bazarow_add_favorites'] == 'Y' && !isset($this->arResult['FATAL_ERROR'])){
			$this->checkFavoritesElement($this->requestData["bazarow_FAVORITES_ELEMENT_ID"]);
		}
	}
	...
	private function checkFavoritesElement($ElementId){
		global $USER, $DB;
		if($ElementId <= 0){
			throw new Main\SystemException(Loc::getMessage('bazarow_FAVORITES_NOT_FIND_ELEMENT'));
		}
		...
		if($favorItem = $favorDb->fetch()){
			/** delete favor */
			$DB->StartTransaction();
			$result = \bazarow\Favorites\FavoritesTable::delete($favorItem['ID']);
			...
			$result = \bazarow\Favorites\FavoritesTable::add($arFields);
			if($result->isSuccess()){
