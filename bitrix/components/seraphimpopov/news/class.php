...
class NewsListComponent extends CBitrixComponent
{
    ...
    private function getNewsList()
    {
        ...
        $result = ElementTable::getList([
            "select" => ["ID", "NAME", "PREVIEW_TEXT", 'DETAIL_PAGE_URL' => 'IBLOCK.DETAIL_PAGE_URL'],
            "filter" => ["IBLOCK_ID" => 47, "ACTIVE" => "Y"],  // Укажите ID вашего инфоблока
        ]);

        while ($item = $result->fetch()) {
            echo "<pre>"; print_r($item['DETAIL_PAGE_URL']); echo "<pre/>";
